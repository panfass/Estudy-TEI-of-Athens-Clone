...
String msg = RESTPost.httpPost( "http://localhost:8080/login/rest-api/check/", xmlInput, "xml" );
int userID = XMLParser.getUserID( msg );
...
if( userID != -1 ){
	...
	si.setUserID( userID );
	xmlInput = JavaObjectToXMLConversion.convert( si );

	msg = RESTPost.httpPost( "http://localhost:8080/login/rest-api/getRoleID/", xmlInput, "xml" );
	int roleID = XMLParser.getRoleID( msg );

	if( roleID == 1 ) {%>
		
		<%
		response.sendRedirect( "Foithths.jsp" );
		return;  
	}
